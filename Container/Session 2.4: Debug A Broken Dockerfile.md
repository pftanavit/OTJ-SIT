...
1. Examine the given code and try running it.
    ...
    ```
    The code failed. After closely examining it, notice `WORKDIR` redirected to `/app`. However, `COPY` copied `start.sh` into `/scripts/start.sh`. So, when `RUN`, it cannot find `start.sh` since it is in `/script` but right now we are in `/app`.
...
2. Try starting a container now.
    ...
    ```
    What happened. During `CMD ["start.sh"]`, it failed to execute. When using `CMD`, Docker bypasses the Linux shell and tries to invoke the command directly.The operating system looks at `start.sh` and assumes it is a global system command, however, `start.sh` lives in `/app` so it failed to find it and crashes.
