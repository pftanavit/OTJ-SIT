...
## 3. Network 🗺️
The Network Layer manages multi-network routing and logical destination addressing across the global internet infrastructure. It encapsulates Transport Layer segments into **Packets**, writes source and destination IP addresses onto the headers, and runs routing protocols to calculate the most efficient path across entirely separate networks.
...
## 2. Data Link 🚚
The Data Link Layer directs the physical delivery of data frames between two directly adjacent hardware devices operating on the exact same local network (LAN). It packs network packets into hardware-ready **Frames**, marks them with permanent physical **MAC Addresses**. It uses a tail-end Frame Check Sequence (FCS) to detect localized errors and coordinates media access control so multiple devices do not talk over each other.
...
2. MAC (Media Access Control): The lower sublayer. It talks to the Physical Layer, manages physical addressing, and controls hardware access to the media.
...
## 1. Physical ⚡
The Physical Layer governs the raw physical hardware, mechanical connectors, and signal pathways that transport raw data streams. It translates higher-level logical data structures into unformatted, binary bitstreams (`1s and 0s`). Then converts them into measurable physical phenomena such as electrical voltages, light pulses, or radio frequencies without assigning any logic or meaning to the data.
